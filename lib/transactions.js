...
exports.CreateGeneration = function(rpcData, publicKey, extraNoncePlaceholder, reward, txMessages, recipients){
    ...
    var txInPrevOutHash = Buffer.alloc(32);
    ...
    var p1 = Buffer.concat([
        ...
        // Remove txTimestamp as it's not needed for most coins
        // If specific coins need timestamp, it should be passed through rpcData
    ...
    // Handle special transaction extra payload (for coins like Dash)
    if (rpcData.extraPayload) {
        var extraPayload = Buffer.from(rpcData.extraPayload, 'hex');
        p2 = Buffer.concat([
            p2,
            util.varIntBuffer(extraPayload.length),
            extraPayload
        ]);
    }
