...
function DaemonInterface(daemons, logger){
    ...
    function isOnline(callback){
        cmd('getinfo', [], function(results){
            // Check if all daemons returned "Method not found"
            var allMethodNotFound = results.every(function(result){
                return result.error && result.error.code === -32601;
            });
            if (allMethodNotFound) {
                // Try new RPCs
                cmd('getblockchaininfo', [], function(bcResults){
                    cmd('getnetworkinfo', [], function(netResults){
                        var allOnline = bcResults.every(function(result){
                            return !result.error;
                        }) && netResults.every(function(result){
                            return !result.error;
                        });
                        callback(allOnline);
                        if (!allOnline)
                            _this.emit('connectionFailed', bcResults.concat(netResults));
                    });
                });
            } else {
