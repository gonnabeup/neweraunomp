...
function DaemonInterface(daemons, logger){
    ...
    function isOnline(callback){
        cmd('getblockchaininfo', [], function(results){
            // First check if we can connect using modern RPC methods
            var allOnline = results.every(function(result){
                return !result.error || (result.error && result.error.code === -32601);
            });

            if (allOnline) {
                // Try modern methods batch
                var modernCalls = [
                    ['getblockchaininfo', []],
                    ['getnetworkinfo', []],
                    ['getmininginfo', []],
                ...
                batchCmd(modernCalls, function(error, results){
                    if (!error) {
                        callback(true);
                    } else {
                        ...
                        batchCmd(legacyCalls, function(error, results){
                            var allOnline = !error;
                            callback(allOnline);
                            if (!allOnline)
    ...
    function performHttpRequest(instance, jsonData, callback){
        ...
        var parseJson = function(res, data){
            ...
            if (dataJson) {
                // Ensure response data is properly initialized
                if (dataJson.result === undefined) {
