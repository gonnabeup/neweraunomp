        ...
        _this.jobManager.on('newBlock', function(blockTemplate){
            ...
            if(!shareData.error) {
                for(var i = 0; i < _this.auxes.length; i++) {
                    // Only process auxes with valid rpcData and target
                    if(_this.auxes[i].rpcData && _this.auxes[i].rpcData.target && _this.auxes[i].rpcData.target.ge(shareData.blockBigNum)) {
                        if (blockHexInvalid) { // Defensive: ensure blockHexInvalid is defined
                            SubmitAuxBlock(i, Buffer.from(blockHexInvalid, 'hex').slice(0,80), shareData.coinbaseBuffer, shareData.headerHash, auxResult);
                        } else {
                            emitErrorLog('blockHexInvalid is undefined for aux ' + i);
                        }
                    }
                }
            }
