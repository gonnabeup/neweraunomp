...
var pool = module.exports = function pool(options, authorizeFn){
    ...
    function SubmitAuxBlock(aux, headerBuffer, coinbaseBuffer, blockHash, callback) {
        if (!_this.auxes[aux] || !_this.auxes[aux].rpcData || !_this.auxes[aux].rpcData.hash) {
            emitErrorLog('Cannot submit aux block - invalid aux data');
            return;
        }

        try {
            var branchProof = _this.jobManager.auxMerkleTree.getHashProof(
                util.uint256BufferFromHash(_this.auxes[aux].rpcData.hash)
            );
            if(!branchProof) branchProof = Buffer.concat([util.varIntBuffer(0), util.packInt32LE(0)]);

            if (!headerBuffer || !coinbaseBuffer || !blockHash) {
                emitErrorLog('Cannot submit aux block - missing required buffers');
                return;
            }

            var coinbaseProof = Buffer.concat([
            ...
            emitErrorLog('Error creating aux submission: ' + e.message);
            callback && callback(null, aux);
        }
    }
        ...
        _this.jobManager.on('newBlock', function(blockTemplate){
            ...
            if(!shareData.error) {
                for(var i = 0; i < _this.auxes.length; i++) {
                    // More thorough validation of aux data
                    if(_this.auxes[i] && 
                       !_this.auxes[i].inactive && 
                       _this.auxes[i].rpcData && 
                       _this.auxes[i].rpcData.target && 
                       _this.auxes[i].rpcData.hash && 
                       _this.auxes[i].rpcData.target.ge(shareData.blockBigNum)) {
                    
                        if (!blockHexInvalid) {
                            emitWarningLog('Skipping aux submission - blockHexInvalid is undefined');
                            continue;
                        }
                        
                        try {
                            SubmitAuxBlock(i, Buffer.from(blockHexInvalid, 'hex').slice(0,80), 
                                          shareData.coinbaseBuffer, 
                                          shareData.headerHash, 
                                          auxResult);
                        } catch (e) {
                            emitErrorLog('Failed to submit aux block: ' + e.message);
                        }
                    }
                }
            }
    ...
    function DetectCoinData(finishedCallback){
        ...
        _this.daemon.batchCmd(batchRpcCallsNew, function(error, results){
            let useLegacy = false;
            if (error || !results) {
                useLegacy = true;
            } else {
                // If any of the new RPCs are not found, fallback to legacy
                for (let i = 0; i < results.length; i++) {
                    if (results[i] && results[i].error && results[i].error.code === -32601) {
                        useLegacy = true;
                        break;
                    }
                }
            ...
            if (useLegacy) {
                // Fallback to legacy getinfo
                var batchRpcCallsLegacy = [
                    ['validateaddress', [options.address]],
                    ['getdifficulty', []],
                    ['getinfo', []],
                    ['getmininginfo', []],
                ...
                _this.daemon.batchCmd(batchRpcCallsLegacy, function(errorLegacy, resultsLegacy){
                    if (errorLegacy || !resultsLegacy){
                        console.log(resultsLegacy)
                        emitErrorLog('Could not start pool, error with init batch RPC call: ' + JSON.stringify(errorLegacy));
                        return;
                    }
    
                    var rpcResults = {};
                    for (var i = 0; i < resultsLegacy.length; i++){
                        var rpcCall = batchRpcCallsLegacy[i][0];
                        var r = resultsLegacy[i];
                    ...
                    if (rpcResults.submitblock && rpcResults.submitblock.message === 'Method not found'){
                        options.hasSubmitMethod = false;
                    }
                    else if (rpcResults.submitblock && rpcResults.submitblock.code === -1){
                        options.hasSubmitMethod = true;
                    }
                    else {
                        emitErrorLog('Could not detect block submission RPC method, ' + JSON.stringify(resultsLegacy));
                        return;
                    }
            ...
            var rpcResults = {};
            for (var i = 0; i < results.length; i++){
                var rpcCall = batchRpcCallsNew[i][0];
                var r = results[i];
                rpcResults[rpcCall] = r.result || r.error;
                if (rpcCall !== 'submitblock' && (r.error || !r.result)){
                    emitErrorLog('Could not start pool, error with init RPC ' + rpcCall + ' - ' + JSON.stringify(r.error));
            ...
            options.testnet = rpcResults.getblockchaininfo.chain === 'test' || rpcResults.getblockchaininfo.testnet;
        ...
        _this.daemon.cmd('getblocktemplate',
            ...
            function(result){
                if (result.error){
                    ...
                    data.auxes = [];
                    // Only add valid aux data
                    for(var i = 0; i < _this.auxes.length; i++) {
                        if (_this.auxes[i] && _this.auxes[i].rpcData && !_this.auxes[i].inactive) {
